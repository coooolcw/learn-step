...
实例方法entries(),keys()和values()——用于遍历数组  
它们都返回一个遍历器对象,可以用for...of循环进行遍历,keys()是对键名的遍历,values()是对键值的遍历,entries()是对键值对的遍历  
  
实例方法copyWithin().在当前数组内部,将指定位置的成员复制到其他位置(会覆盖原有成员),然后返回当前数组.会修改现有数组  
